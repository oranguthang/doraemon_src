...
class RuntimeValidationTests(unittest.TestCase):
    def test_selects_profile_specific_rom_hash(self) -> None:
        document = {
            "rom_sha1": "original",
            "rom_sha1_by_profile": {
                "original": "original",
        ...
        self.assertEqual(RUNNER.expected_rom_sha1(document, "rev_a"), "revision-a")
        with self.assertRaisesRegex(ValueError, "no missing ROM hash"):
            RUNNER.expected_rom_sha1(document, "missing")

    def test_legacy_runtime_hash_is_original_only(self) -> None:
        document = {"rom_sha1": "original"}
        self.assertEqual(RUNNER.expected_rom_sha1(document, "original"), "original")
        with self.assertRaisesRegex(ValueError, "no revision profile hashes"):
            RUNNER.expected_rom_sha1(document, "rev_a")

    def test_profile_override_replaces_only_declared_scenario_fields(self) -> None:
        scenario = {
            "id": "world3",
            "max_frames": 900,
            "inputs": ["shared"],
            "profile_overrides": {"rev_a": {"max_frames": 500}},
        }
        resolved = RUNNER.resolve_scenario(scenario, "rev_a")
        self.assertEqual(resolved["max_frames"], 500)
        self.assertEqual(resolved["inputs"], ["shared"])
        self.assertNotIn("profile_overrides", resolved)

    def test_requires_revision_reset_after_named_probe(self) -> None:
        scenario = {
            "expected_reset": {
                "after_probe": "changed_helper",
                "bank": 2,
                "selector": "0A",
                "maximum_frame_gap": 0,
            }
        ...
        rows = [
            row("probe", detail="changed_helper", bank="2", selector="0A", frame="20"),
            row("reset", bank="2", selector="0A", frame="20"),
        ]
        self.assertTrue(RUNTIME.reset_after_probe(scenario, rows))
        rows[-1]["frame"] = "21"
        self.assertFalse(RUNTIME.reset_after_probe(scenario, rows))
